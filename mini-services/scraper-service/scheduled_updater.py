#!/usr/bin/env python3
"""
🔄 نظام التحديث المجدول - Scheduled Data Updater
يشتغل على فترات ويُحدّث قاعدة البيانات
"""
import subprocess
import json
import sqlite3
...
LOG_FILE = '/home/z/my-project/mini-services/scraper-service/scraper.log'
...
def open_page(url):
    ...
    time.sleep(2)
    return 'error' not in result.stdout.lower() and 'غير موجودة' not in result.stdout
...
def save_data(symbol, exchange, data):
    ...
    try:
        ...
        if not row:
            c.execute('''INSERT INTO Stock (id, symbol, nameEn, sector, industry, isin, exchange, iconUrl, createdAt, updatedAt)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                (stock_id, symbol, '', ov.get('sector', ''), ov.get('industry', ''), ov.get('isin', ''),
                 exchange, data.get('iconPath', ''), now, now))
        else:
            c.execute('UPDATE Stock SET sector=?, industry=?, isin=?, iconUrl=?, updatedAt=? WHERE id=?',
                (ov.get('sector', ''), ov.get('industry', ''), ov.get('isin', ''), data.get('iconPath', ''), now, stock_id))
...
def update_exchange(exchange, batch_size=20):
    """تحديث دفعة من الأسهم"""
    log(f"📊 بدء تحديث {exchange}")
    
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    
    # نجيب الأسهم اللي محتاجة تحديث
    c.execute(f'''
        SELECT s.symbol FROM Stock s
        LEFT JOIN TechnicalIndicators t ON s.id = t.stockId
        WHERE s.exchange = '{exchange}' AND t.id IS NULL
        ORDER BY s.symbol
        LIMIT {batch_size}
    ''')
    ...
    if not stocks:
        log(f"✅ {exchange}: كل الأسهم محدثة")
        return 0
    
    log(f"   عدد الأسهم للتحديث: {len(stocks)}")
    ...
    for i, symbol in enumerate(stocks):
        ...
        if open_page(url):
            ...
            if save_data(symbol, exchange, data):
                success += 1
                log(f"   [{i+1}/{len(stocks)}] {symbol}: ✓")
            else:
                log(f"   [{i+1}/{len(stocks)}] {symbol}: ✗ حفظ")
        else:
            log(f"   [{i+1}/{len(stocks)}] {symbol}: ✗ صفحة")
        
        time.sleep(1)
    
    close_browser()
    log(f"✅ {exchange}: تم تحديث {success}/{len(stocks)}")
    return success

def run_scheduled_update():
    """تشغيل التحديث المجدول"""
    log("="*50)
    log("🔄 بدء التحديث المجدول")
    log("="*50)
    ...
    for exchange in exchanges:
        updated = update_exchange(exchange, batch_size=10)
        total += updated
        time.sleep(5)
    
    log("="*50)
    log(f"🎉 انتهى التحديث: {total} سهم")
    log("="*50)
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == '--schedule':
            ...
            while True:
                ...
                log("⏰ انتظار 1 ساعة للتحديث التالي...")
                time.sleep(3600)
            ...
            update_exchange(exchange, batch)
    else:
        run_scheduled_update()
